...
def seed(cursor):
    ...
    # Lấy đúng danh sách học viên từ DB
    cursor.execute("""
    ...
    # Lấy các gói học ACTIVE từ khóa học ACTIVE
    cursor.execute("""
        SELECT cp.id, cp.course_id, cp.name, cp.price, cp.original_price, cp.delivery_mode
    ...
    # Seed đơn hàng thực tế cho 50 học viên (mỗi học viên 2-3 đơn)
    for user in users:
        user_id = user["id"]
        num_orders = random.randint(2, 3)
        chosen_pkgs = random.sample(packages, k=min(num_orders, len(packages)))
